...
func (o *OCR) GetCustomers() ([]Customer, []byte, error) {
	...
	c, err = readCustomers(customersfile)
	return c, body, err
}

func readCustomers(customersfile []byte) ([]Customer, error) {
	var err error
	var c []Customer
...
func (o *OCR) GetUsers() ([]User, []byte, error) {
	...
	url = fmt.Sprintf("%s&payload[start_date]=%s&payload[end_date]=%s", url, t.Format(layout), t.Format(layout))
